/////////////////////////////////////////
// if you came into this project with  //
// the expectation of nice clean code  //
// im really really sorry			   //
//									   //
// lord forgive me for all the _global.//
// spam, im really really sorry		   //
/////////////////////////////////////////
class com.clubpenguin.games.dancing.MenuSystem
{
    ...
    function loadMenu($menuID)
    {
        ...
        switch (currentMenu)
        {
            ...
            case com.clubpenguin.games.dancing.MenuSystem.MENU_WELCOME_OPTIONS:
            {
				...
				_loc3.push("OSU! - I'd like to play an osu!mania song!");
			...
			case com.clubpenguin.games.dancing.MenuSystem.MENU_OSU_URL:
            {
                movie[com.clubpenguin.games.dancing.MenuSystem.MOVIE_HOST_COMPERE].gotoAndStop(com.clubpenguin.games.dancing.MenuSystem.ANIM_FRAME_WAIT);
                movie[com.clubpenguin.games.dancing.MenuSystem.MOVIE_HOST_COMPERE].animation.movie.gotoAndPlay("talkStart");
                var osuUrlMenuOptions = new Array();
                osuUrlMenuOptions.push("This is the song I want to play");
                osuUrlMenuOptions.push(com.clubpenguin.util.LocaleText.getText("menu_song_item_back"));
                this.showSpeechBubble("You chose to dance to an osu!mania song...\nPaste a link to your osu! beatmap below!");
                this.showMenuOptions(osuUrlMenuOptions);
				_global.movie.options.urlInput._visible = true;
                movie[com.clubpenguin.games.dancing.MenuSystem.MOVIE_HOW_TO_PLAY_ANIM]._visible = false;
                trace("target path " + targetPath(movie[com.clubpenguin.games.dancing.MenuSystem.MOVIE_HOW_TO_PLAY_ANIM]));
                movie[com.clubpenguin.games.dancing.MenuSystem.MOVIE_HOW_TO_PLAY_ANIM].gotoAndStop(1);
                break;
            } 
    ...
    function handleClick($buttonID)
    {
		...
		if (_global.keyListener == com.clubpenguin.games.dancing.MenuSystem.MENU_SECRET_KONAMI) {
			switch ($buttonID)
			{
				case 1:
				{
					getURL('https://github.com/Thestickman391/Dance-Contest-Plus', '_blank');
					return;
				} 
				case 2:
				{
    ...
    function showMenuOptions($menuOptions)
    {
		...
		if (currentMenu == com.clubpenguin.games.dancing.MenuSystem.MENU_OSU_URL)
		{
			movie[com.clubpenguin.games.dancing.MenuSystem.MOVIE_OPTIONS_MENU][com.clubpenguin.games.dancing.MenuSystem.MOVIE_OPTIONS_BACKGROUND]._height = ($menuOptions.length + 1) * _loc4 + ($menuOptions.length) * _loc6 + _loc5;
			movie[com.clubpenguin.games.dancing.MenuSystem.MOVIE_OPTIONS_MENU][com.clubpenguin.games.dancing.MenuSystem.MOVIE_OPTIONS_BACKGROUND]._y = (-30.5 * ($menuOptions.length + 1)) + 122;
		}
		else
		{
			...
			movie[com.clubpenguin.games.dancing.MenuSystem.MOVIE_OPTIONS_MENU][com.clubpenguin.games.dancing.MenuSystem.MOVIE_OPTIONS_BACKGROUND]._y = (-30.5 * ($menuOptions.length)) + 122;
		}
        var _loc2 = 0;
        while (_loc2++ < 9)
        {
        ...
        for (var _loc2 = 0; _loc2 < $menuOptions.length; ++_loc2)
        {
            ...
            movie[com.clubpenguin.games.dancing.MenuSystem.MOVIE_OPTIONS_MENU][com.clubpenguin.games.dancing.MenuSystem.MOVIE_OPTIONS_ITEM + (_loc2 + 1)]._y = (_loc2 - 1) * 30.5 - ((($menuOptions.length - 5) * 30.5) - 7);
        } // end of for
    ...
    static var MENU_OSU_URL = 691;
    static var MENU_OSU_SELECT = 692;
    static var MENU_SECRET_KONAMI = 693;
